...
// GET /mahasiswa
router.get('/', (req,res) => {
    db.query('SELECT * FROM mahasiswiJ', (error, results) => {
        if (error) {
...
router.get('/:nim', (req, res) => {
    ...
    db.query('SELECT * FROM mahasiswiJ WHERE nim = ?', [mahasiswa], (error, results) => {
        if (error) {
            console.error('Error fetching mahasiswa:', error);
            res.status(500).json({ message: 'Internal Server Error' });
...
router.put('/:nim', (req, res) => {
    ...
    db.query('UPDATE mahasiswiJ SET nama = ?,gender = ?,prodi = ?,alamat = ? WHERE nim = ?' ,
...
router.post("/:nim", (req, res) => {
    ...
    const { nama, gender, prodi, alamat } = req.body;
    db.query("INSERT INTO mahasiswiJ (nim, nama, gender, prodi, alamat) VALUES (?, ?, ?, ?, ?)", [nim, nama, gender, prodi, alamat], (error) => {
      if (error) {
  ...
  router.delete("/:nim", (req, res) => {
    const nim = req.params.nim;
    db.query("DELETE FROM mahasiswiJ WHERE nim = ?", [nim], (error) => {
      if (error) {
